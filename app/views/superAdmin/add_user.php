...
$options = '<option default>Select Devision</option>';
...
$roleOptions = '<option default>Select Role</option>';

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $roleOptions .= '<option value="' . htmlspecialchars($row['name']) . '">' . htmlspecialchars($row['name']) . '</option>';
    }
} else {
    $roleOptions .= '<option>No job roles available</option>';
}
